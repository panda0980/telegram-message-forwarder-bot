...
try:
  ...
  sudo_users = list(set(x for x in environ.get("SUDO_USERS", "1983530070").split(";")))
  try:
...
if tg_session:
  ...
  app = Client(":memory:", api_id=api_id, api_hash=api_hash, session_string = tg_session)
elif bot_token:
  LOG.info("Bot Mode")
  app = Client(api_id, api_hash, bot_token=bot_token)
else:
